...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        function previewFile(fileName, fileUrl, folder) {
            $.ajax({
                url: simpleFileManagerAjax.ajax_url, type: "POST", data: {
                    action: "simple_file_preview", nonce: simpleFileManagerAjax.nonce, file: fileName, folder: folder,
                }, beforeSend: function () {
                    ...
                    if (response.success) {
                        ...
                        if (fileType === 'image') {
                            ...
                            previewHtml = '<div class="simple-unsupported-preview">' + '<p>No preview available for this file type.</p>' + '<a href="' + data.url + '" class="simple-button" target="_blank">Download File</a>' + '</div>';
                        }
                        ...
                        // Show file details
                        var detailsHtml = "<p>Type: " + (data.type || "Unknown") + "</p>" + "<p>Size: " + data.size + "</p>";
        ...
        function displayFiles(files) {
            ...
            var uploadAreaHtml = '<div class="simple-persistent-upload">' + '<h4 class="simple-persistent-upload-title">Upload to this folder</h4>' + '<div class="simple-persistent-upload-actions">' + '<button id="simple-persistent-upload-btn" class="simple-button">' + "<span>⬆️</span> Upload Files</button>" + // '<button id="simple-persistent-import-url" class="simple-button">' +
            ...
            $.each(files, function (index, file) {
                ...
                html += '<div class="simple-file-item">' + '<div class="simple-file-preview">' + previewHtml + '</div>' + '<div class="simple-file-info">' + '<div class="simple-file-name">' + file.name + '</div>' + '<div class="simple-file-size">' + file.size + '</div>' + '</div>' + '<div class="simple-file-actions">' + '<button class="simple-file-action simple-file-preview" data-file="' + file.name + '" data-url="' + file.url + '">👁️</button>' + '<button class="simple-file-action simple-file-rename" data-file="' + file.name + '">✏️</button>' + '<button class="simple-file-action simple-file-download" data-url="' + file.url + '">⬇️</button>' + '<button class="simple-file-action simple-file-delete" data-file="' + file.name + '">🗑️</button>' + '<button class="simple-file-action simple-file-share" data-file="' + file.name + '">🔗</button>' + '</div>' + '</div>';
            });
        ...
        function createFolder(folderName) {
            $.ajax({
                url: simpleFileManagerAjax.ajax_url, type: "POST", data: {
                    action: "simple_create_folder", nonce: simpleFileManagerAjax.nonce, folder_name: folderName,
                }, beforeSend: function () {
                    $("#simple-create-folder").prop("disabled", true).text("Creating...");
                }, success: function (response) {
                    if (response.success) {
                        // Add folder to list
                        var folderHtml = '<li><a href="#" data-folder="' + response.data.folder + '">' + "📁 " + response.data.folder.charAt(0).toUpperCase() + response.data.folder.slice(1) + "</a>" + '<div class="simple-folder-options">' + '<button class="simple-folder-options-button">⋮</button>' + '<div class="simple-folder-options-menu">' + '<a href="#" class="simple-folder-rename-option" data-folder="' + response.data.folder + '">' + "<span>✏️</span> Rename" + "</a>" + '<a href="#" class="simple-folder-delete-option delete-option" data-folder="' + response.data.folder + '">' + "<span>🗑️</span> Delete" + "</a>" + "</div>" + "</div>" + "</li>";
        ...
        function deleteFile(fileName, folder) {
            $.ajax({
                url: simpleFileManagerAjax.ajax_url, type: "POST", data: {
                    action: "simple_delete_file", nonce: simpleFileManagerAjax.nonce, file: fileName, folder: folder,
                }, beforeSend: function () {
        ...
        function deleteFolder(folderName) {
            $.ajax({
                url: simpleFileManagerAjax.ajax_url, type: "POST", data: {
                    action: "simple_delete_folder", nonce: simpleFileManagerAjax.nonce, folder_name: folderName,
                }, beforeSend: function () {
                    // Add loading indicator if needed
                }, success: function (response) {
                    if (response.success) {
